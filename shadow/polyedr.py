...
class Polyedr:
    ...
    def __init__(self, file):
        ...
        # Сумма длин видимых рёбер
        self.summ_vis_edges = 0.0
    ...
    # Метод, считающий сумму длин ребер, удовлетворяющих условиям
    def visual_edge(self):
        self.edges_uniq()
        for e in self.edges:
            for f in self.facets:
                e.shadow(f)
            center_point = R3((e.beg.x + e.fin.x) / 2 / self.c,
                              (e.beg.y + e.fin.y) / 2 / self.c, 0)
            angle_check = e.correct_angle()
            if 1 < center_point.x < 3 and angle_check:
                if len(e.gaps) == 1:
                    self.summ_vis_edges += sqrt((e.fin - e.beg).dot(
                        e.fin - e.beg)) / self.c
        return self.summ_vis_edges
